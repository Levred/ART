...
class XcfilterVisitor(NodeVisitor):
    ...
    def visit_block(self, node, visited_children):
        ...
        if isinstance(eval_expr, str):
            ...
            return self.elif_else_or_nothing(elifs,else_)
    ...
    def elif_else_or_nothing(self,elifs:list[tuple[str,bool]],else_:list):
        fini = False
        i = 0
        while not fini :
            if i == len(elifs):
        ...
        if i != len(elifs):
            ...
            if else_ != '':
                contenu = "".join([k[0] for k in elifs[i:]] + list(else_))
            else : 
                contenu = "".join([k[0] for k in elifs[i:]])
            return contenu
        else:
